...
def get_user_text(message):
	...
	if message.text.lower() == 'привет':
		...
		create = types.KeyboardButton('Создать заметку')
		view = types.KeyboardButton('Посмотреть заметки')
		back = types.KeyboardButton('Назад')

		markup.add(back, create, view)
		bot.send_message(message.chat.id, text='даб', reply_markup=markup)

	elif message.text == 'Создать заметку':
		pass
...
#Менять проверяемую почту
#	проверить почту --> меняються данные в глобальных переменных и подставляються в функцию
